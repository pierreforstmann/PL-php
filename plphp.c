...
// PHP 8: PACKAGE_URL already defined 
// #include "config.h"
...
#define INI_HARDCODED(name,value) \
	zend_alter_ini_entry( \
        zend_string_init(name, sizeof(name)-1, 0), \
        zend_string_init(value, strlen(value), 0), \
...
// begin - PHP 8
//
PHP_MINIT_FUNCTION(plphp);
PHP_MSHUTDOWN_FUNCTION(plphp);
PHP_MINFO_FUNCTION(plphp); 

// This is the struct that should contain the reference
zend_module_entry plphp_module_entry = {
    STANDARD_MODULE_HEADER,
    "plphp",
    plphp_functions, // <--- Functions are registered here automatically
    PHP_MINIT(plphp),
    PHP_MSHUTDOWN(plphp),
    NULL,
    NULL,
...
static void plphp_error_cb(int type, zend_string *filename, const uint32_t lineno,
								  zend_string *message);
	...
	PG_TRY();
	{
		zend_try
		{
			...
			if (php_module_startup(&plphp_sapi_module, NULL) == FAILURE)
				elog(ERROR, "php_module_startup call failed, NULL");
...
plphp_validator(PG_FUNCTION_ARGS)
{
	...
	PG_TRY();
	{
		...
		zend_try
		{
			 ...
			 */
			// PHP 8
			 key = zend_string_init(tmpname, strlen(tmpname), 0);
			 zend_hash_del(CG(function_table), key);
			 zend_string_release(key);
			// zend_hash_del(CG(function_table), zend_string_init(tmpname, strlen(tmpname), 0));
			// PHP 8
...
plphp_trig_build_args(FunctionCallInfo fcinfo)
{
	...
	// PHP 8: remove last argument of add_assoc_string
	/* The basic variables */
	...
	if (tdata->tg_trigger->tgnargs > 0)
	{
		...
		for (i = 0; i < tdata->tg_trigger->tgnargs; i++)
			// PHP 8: remove last arg.
			add_index_string(hashref, i, tdata->tg_trigger->tgargs[i]);

		// PHP8: use Z_ARRVAL_P
		zend_hash_update(Z_ARRVAL_P(retval), zend_string_init("args", strlen("args"),0),
						 hashref);
	}

	/* WHEN */
	// PHP 8: remove last parameter of add_assoc_string
...
plphp_compile_function(Oid fnoid, bool is_trigger TSRMLS_DC)
{
	...
	if (prodesc == NULL)
	{
	    ...
	    key = zend_string_init(prodesc->proname, strlen(prodesc->proname), 0);
        zend_hash_del(CG(function_table), key);
        zend_string_release(key);
		// PHP 8 
		...
		lpointer = (char *) palloc(64);
		sprintf(lpointer, "%p", (void *) prodesc);
		// PHP 8: remove last parameter of add_assoc_string
		 ...
		 (j = IS_ARGMODE_OUT(desc->arg_argmode[i]) ? j : j + 1), i++)
	{
		...
		if (desc->arg_typtype[i] == TYPTYPE_COMPOSITE)
		{
			...
			else
			{
				...
				tmptup.t_data = DatumGetHeapTupleHeader(fcinfo->args[j].value);
				...
				//							(void *) &hashref,
				//							sizeof(zval *), NULL);
				zend_hash_next_index_insert(Z_ARRVAL_P(retval), hashref);
					...
					zval *trigdata TSRMLS_DC)
{
	...
	// PHP 8
	// FREE_ZVAL(funcname);
    // no need to free it because stack allocated	
 ...
 */
void
plphp_error_cb(int type, zend_string *filename, const uint32_t lineno,
	   		   zend_string *message)
{
	// PHP 8 message is already formattted
